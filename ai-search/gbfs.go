...
func (ds GreedyBestFirstSearch) GetFrontier() []*Node {
	return ds.Frontier
}

func (ds *GreedyBestFirstSearch) Add(i *Node) {
	i.CostToGoal = i.ManhattanDistance(ds.Game.Goal)
	ds.Frontier.Push(i)
	heap.Init(&ds.Frontier)
}

func (ds *GreedyBestFirstSearch) ContainsState(i *Node) bool {
...
func (ds *GreedyBestFirstSearch) Empty() bool {
	return len(ds.Frontier) == 0
}

func (ds *GreedyBestFirstSearch) Remove() (*Node, error) {
	if ds.Empty() {
...
func (ds *GreedyBestFirstSearch) Solve() {
...
func (ds *GreedyBestFirstSearch) Neighbors(node *Node) []*Node {
	row := node.State.Row
	col := node.State.Col
	candidates := []*Node{
