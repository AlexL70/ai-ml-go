		...
		for _, x := range dfs.Neighbors(currentNode) {
			if !dfs.ContainsState(x) {
				if !inExplored(x.State, dfs.Game.Explored) {
					dfs.Add(&Node{State: x.State, Parent: currentNode, Action: x.Action})
				}
			}
		}
	}
...
func (dfs *DepthFirstSearch) Neighbors(node *Node) []*Node {
	...
	for _, n := range candidates {
		if 0 <= n.State.Row && n.State.Row < dfs.Game.Height &&
			0 <= n.State.Col && n.State.Col < dfs.Game.Width &&
			!dfs.Game.Walls[n.State.Row][n.State.Col].Wall {
